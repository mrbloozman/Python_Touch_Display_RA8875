...
class Control:
	...
	def render(self):
		...
		self._tft.textSetCursor(self._x + int(self._w / 2) - int(self.text_width() / 2),
								self._y + int(self._h / 2) - int(self.text_height() / 2))
...
class Input(Control):
	...
	def render(self):
		...
		# setting for center/middle of rect
		self._tft.textSetCursor(self._x + int(self._w / 2) - int(self.value_width() / 2),
								self._y + int(self._h / 2) - int(self.value_height() / 2))
		self._tft.textEnlarge(self._size)
...
class Spinbox(Input):
	def __init__(self, **kwargs):
		...
		if 'w' not in kwargs:
			self._w = (self._border * 2) + self._label.w() + self._input.w() + self._upBtn.w()

		if 'h' not in kwargs:
			self._h = (self._border * 2) + self._upBtn.h() + self._dnBtn.h()

		self.position()

	def position(self):
		self._label.x(self._x + self._border)
		# self._label.x(self._x + int(self._w / 2) - self._label.w())
		self._label.y(self._y + self._border + int(self._h / 2) - int(self._label.h() / 2))
		self._input.x(self._label.x() + self._label.w())
		# self._input.x(self._x + int(self._w / 2))
		self._input.y(self._y + self._border + int(self._h / 2) - int(self._input.h() / 2))
...
# Listbox is a container for ListboxItem instances - callbacks will be overwritten to control the listbox value
